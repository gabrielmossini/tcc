...
def create(request):
    ...
    return render(request, 'accounts/create.html', {'user_form': user_form, 'profile_form': profile_form,})
...
def view(request):
    ...
    return render(request, 'accounts/view.html', {'users': users, 'page_obj': page_obj})
...
def update_user(request, user_id):
    ...
    return render(request, 'accounts/update.html', {'user_form': user_form, 'profile_form': profile_form, 'user': user, 'user_id': user_id})
...
def users_PDF(request):
    ...
    for user in users:
        ...
        pdf.drawString(margin, y,        f"Nome:               {profile.name if profile else 'N/A'}")
        pdf.drawString(margin, y - 20,   f"Usuário:            {user.username}")
        pdf.drawString(margin, y - 40,   f"Email:              {user.email}")
        pdf.drawString(margin, y - 60,   f"CPF:                {profile.cpf if profile else 'N/A'}")
        pdf.drawString(margin, y - 80,   f"Nascimento:         {profile.birthday if profile else 'N/A'}")
        pdf.drawString(margin, y - 100,  f"Cadastro:           {user.date_joined.strftime('%d/%m/%Y %H:%M')}")
        pdf.drawString(margin, y - 120,  f"Status:             {'Ativo' if user.is_active else 'Inativo'}")
        pdf.drawString(margin, y - 140,  "-------------------------------------------------------")
        y -= 180
